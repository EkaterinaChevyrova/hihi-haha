...
public class TelegramBotService {
    ...
    private void getterJoke(Update update) {
        if (update.message() != null && update.message().text() != null) {
            ...
            if (messageText.equalsIgnoreCase("Хочу хахатушку")) {
                ...
                saveUserAction(chatId, "Хочу хахатушку", randomJoke.getJok());
            }
        }
    }
    ...
    private void sendTopJokesMessage(Long chatId) {
        ...
        Map<String, Long> topJokes = jokesService.countJokesUsage().entrySet().stream()
        ...
        int rank = 1;
        for (Map.Entry<String, Long> entry : topJokes.entrySet()) {
            messageBuilder.append(rank).append(". ").append(entry.getKey()).append(" - ").append(entry.getValue()).append(" раз\n");
            rank++;
        }
    ...
    private void saveUserAction(Long chatId, String action, String joke) {
        ...
        jokesService.saveUserAction(visitor, action, joke);
    }
}
